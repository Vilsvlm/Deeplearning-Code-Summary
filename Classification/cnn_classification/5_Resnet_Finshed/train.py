...
def main():
    ...
    3）target_transform：对图片类别进行预处理的操作，输入为target，输出对其的转换。如果不传该参数，即对target不做任何转换，返回的顺序索引0, 1, 2…；
    ...
    # write dict into json file 编码成json格式方便数据交换
    json_str = json.dumps(cla_dict, indent=4)
    # with 自动调用.colse()释放内存 "w"状态时只写入文件 若文件不存在则创建
    ...
    in_channel = net.fc.in_features  #最后一层的输入通道数
